...
func (self class) IsLooping(node_info Packed.Array[float32]) bool { //gd:AnimationNodeExtension.is_looping
	var r_ret = noescape.CallStatic[bool]( methods.is_looping, gdextension.SizeBool|(gdextension.SizePackedArray<<4), &struct{node_info gdextension.PackedArray[float32]}{pointers.Get(gd.InternalPacked[gd.PackedFloat32Array, float32](node_info))})
	var ret = r_ret
	return ret
}
func (self class) GetRemainingTime(node_info Packed.Array[float32], break_loop bool) float64 { //gd:AnimationNodeExtension.get_remaining_time
	var r_ret = noescape.CallStatic[float64]( methods.get_remaining_time, gdextension.SizeFloat|(gdextension.SizePackedArray<<4)|(gdextension.SizeBool<<8), &struct{node_info gdextension.PackedArray[float32]; break_loop bool}{pointers.Get(gd.InternalPacked[gd.PackedFloat32Array, float32](node_info)), break_loop})
	var ret = r_ret
	return ret
}
...
func (self class) Virtual(name string) reflect.Value {
	switch name {
	case "_process_animation_node": return reflect.ValueOf(self._process_animation_node);
...
func init() {gdclass.Register("AnimationNodeExtension", func(ptr gdreference.Object) any { return Instance{gdclass.NewAnimationNodeExtension(ptr)} })}
