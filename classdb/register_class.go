...
func (class classImplementation) GetVirtual(name gd.StringName) any {
	...
	// Skip codegen-promoted forwarders. After the parent-method-promotion
	// codegen pass (Phase 4), every leaf class's *Extension[T] carries
	// forwarders like `func (o *Extension[T]) GetMinimumSize() Vector2.XY`
	// whose body just calls the engine's get_minimum_size method bind. If
	// such a forwarder shadows a virtual hook of the same Go name
	// (_get_minimum_size → GetMinimumSize), accepting it as the virtual
	// implementation creates an infinite C↔Go recursion: engine asks Go for
	// the virtual, Go forwarder calls engine, engine dispatches the virtual
	// back to Go, … → stack overflow at scene-load. The user is expected to
	// define such overrides directly on their leaf struct; Go's method
	// promotion gives those a real source file/line, while reflect-generated
	// promotion wrappers report "<autogenerated>" — that's the signal we
	// use to filter them out.
	if fn := runtime.FuncForPC(method.Func.Pointer()); fn != nil {
		if file, _ := fn.FileLine(method.Func.Pointer()); strings.HasPrefix(file, "<autogenerated>") {
			return nil
		}
	}
