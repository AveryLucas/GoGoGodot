...
noescape.Call[struct{}]( gd.ObjectChecked(self.AsObject()), methods.connect_node, 0|(gdextension.SizeStringName<<4)|(gdextension.SizeInt<<8)|(gdextension.SizeStringName<<12), &struct{input_node gdextension.StringName; input_index int64; output_node gdextension.StringName}{pointers.Get(gd.InternalStringName(input_node)), input_index, pointers.Get(gd.InternalStringName(output_node))})
}
func (self class) DisconnectNode(input_node String.Name, input_index int64)  { //gd:AnimationNodeBlendTree.disconnect_node
noescape.Call[struct{}]( gd.ObjectChecked(self.AsObject()), methods.disconnect_node, 0|(gdextension.SizeStringName<<4)|(gdextension.SizeInt<<8), &struct{input_node gdextension.StringName; input_index int64}{pointers.Get(gd.InternalStringName(input_node)), input_index})
}
...
func (o *Extension[T]) OnNodeChanged(cb func(node_name string), flags ...Signal.Flags) *Extension[T] {
	var flags_together Signal.Flags
	for _, flag := range flags {
		flags_together |= flag
	}
	gd.ObjectConnect(o.AsObject()[0], gd.NewStringName("node_changed"), gd.NewCallable(cb), int64(flags_together))
	return o
}
...
func (o *Extension[T]) ConnectNode(input_node string, input_index int, output_node string) {
	o.Super().ConnectNode(input_node, input_index, output_node)
}
func (o *Extension[T]) DisconnectNode(input_node string, input_index int) {
	o.Super().DisconnectNode(input_node, input_index)
}
...
func init() {gdclass.Register("AnimationNodeBlendTree", func(ptr gdreference.Object) any { return Instance{gdclass.NewAnimationNodeBlendTree(ptr)} })}
