...
func (o *Extension[T]) OnStateStarted(cb func(state string), flags ...Signal.Flags) *Extension[T] {
	var flags_together Signal.Flags
	for _, flag := range flags {
		flags_together |= flag
	}
	gd.ObjectConnect(o.AsObject()[0], gd.NewStringName("state_started"), gd.NewCallable(cb), int64(flags_together))
	return o
}
func (o *Extension[T]) OnStateFinished(cb func(state string), flags ...Signal.Flags) *Extension[T] {
	var flags_together Signal.Flags
	for _, flag := range flags {
		flags_together |= flag
	}
	gd.ObjectConnect(o.AsObject()[0], gd.NewStringName("state_finished"), gd.NewCallable(cb), int64(flags_together))
	return o
}
