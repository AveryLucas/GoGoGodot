...
func From[X Int.Any | Float.Any, T ~struct{ X, Y, Z, W X }](value T) XYZW {
	var underlying = struct{ X, Y, Z, W X }(value)
	return XYZW{Float.X(underlying.X), Float.X(underlying.Y), Float.X(underlying.Z), Float.X(underlying.W)}
}
